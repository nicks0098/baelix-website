...
export function SiteMotion() {
  useEffect(() => {
    ...
    const marketingSite = document.querySelector<HTMLElement>('.marketing-site');
    ...
    revealGroups.forEach((selector) => {
      document.querySelectorAll<HTMLElement>(selector).forEach((target, index) => {
        target.style.setProperty('--reveal-delay', `${Math.min(index, 4) * 90}ms`);
      });
    });
    ...
    const readyFrame = requestAnimationFrame(() => marketingSite.classList.add('page-ready'));
    ...
    const updatePointer = (event: PointerEvent) => {
      ...
      if (tiltTarget) {
        ...
        tiltTarget.style.setProperty('--tilt-shift-x', `${(x * 8).toFixed(1)}px`);
        tiltTarget.style.setProperty('--tilt-shift-y', `${(y * 8).toFixed(1)}px`);
      }

      const magneticTarget = closestElement(event.target, '.magnetic-cta');
      if (lastMagnetic && lastMagnetic !== magneticTarget) resetMagnetic(lastMagnetic);
      lastMagnetic = magneticTarget;
      if (magneticTarget) {
        const bounds = magneticTarget.getBoundingClientRect();
        magneticTarget.style.setProperty('--magnetic-x', `${((event.clientX - bounds.left - bounds.width / 2) * 0.11).toFixed(1)}px`);
        magneticTarget.style.setProperty('--magnetic-y', `${((event.clientY - bounds.top - bounds.height / 2) * 0.11).toFixed(1)}px`);
      }
    };

    const onPointerOver = (event: PointerEvent) => {
      if (closestElement(event.target, 'a, button, [data-tilt]')) root.classList.add('pointer-interactive');
    };
    ...
    const onPointerOut = (event: PointerEvent) => {
      ...
      if (!related?.closest('a, button, [data-tilt]')) root.classList.remove('pointer-interactive');
    };
    ...
    const updateScroll = () => {
      ...
      scrollFrame = requestAnimationFrame(() => {
        ...
        const heroProgress = clamp(window.scrollY / Math.max(window.innerHeight * 0.92, 1));
        root.style.setProperty('--scroll-progress', `${progress}`);
        root.style.setProperty('--hero-scroll', `${heroProgress}`);
        root.style.setProperty('--hero-scroll-y', `${(heroProgress * 34).toFixed(1)}px`);
        root.style.setProperty('--hero-scroll-scale', `${(1 - heroProgress * 0.045).toFixed(3)}`);
        root.style.setProperty('--hero-scroll-tilt', `${(heroProgress * 2.4).toFixed(2)}deg`);
        ...
        if (experience) {
          ...
          const experienceProgress = clamp((window.innerHeight * 0.78 - bounds.top) / Math.max(bounds.height, 1));
          root.style.setProperty('--experience-scroll', `${experienceProgress}`);
        }
        scrollFrame = 0;
      });
    };
